    ...
    public function index(Request $request)
    {
        ...
        if ($searchTerm !== '') {
            ...
            $posts = BlogPosts::where('published', true)
                ->where(function ($query) use ($searchTerm, $likeOperator) {
                    $query->where('title', $likeOperator, '%'.$searchTerm.'%')
                        ->orWhere('body', $likeOperator, '%'.$searchTerm.'%');
                })->simplePaginate(6);
            ...
            $resolvedCategory = BlogCategories::query()
                ->get(['name'])
                ->first(function ($record) use ($category) {
                    return Str::slug((string) $record->name) === Str::slug($category)
                        || strcasecmp((string) $record->name, $category) === 0;
                });

            $posts = BlogPosts::GetCategories($resolvedCategory?->name ?? $category);  // Already Paginated in BlogPosts Model
