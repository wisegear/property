    ...
    <div class="flex flex-col gap-5">
                ...
                <div class="rounded-lg border border-zinc-200 p-4">
                    ...
                    <div class="mt-3 grid grid-cols-2 gap-3 text-sm">
                        <div>
                            ...
                            <p class="mt-1 text-2xl font-semibold text-zinc-900">{{ number_format(data_get($admin_analytics, "periods.{$period}.visitors", 0)) }}</p>
                        </div>
                        <div>
                            <p class="text-zinc-500">Page views</p>
                            <p class="mt-1 text-2xl font-semibold text-zinc-900">{{ number_format(data_get($admin_analytics, "periods.{$period}.page_views", 0)) }}</p>
                        </div>
                        <div>
                            <p class="text-zinc-500">Events</p>
                            <p class="mt-1 text-lg font-semibold text-zinc-900">{{ number_format(data_get($admin_analytics, "periods.{$period}.events", 0)) }}</p>
                        </div>
                        <div>
                            <p class="text-zinc-500">Bots</p>
                            <p class="mt-1 text-lg font-semibold text-zinc-900">{{ number_format(data_get($admin_analytics, "periods.{$period}.bot_visits", 0)) }}</p>
                        </div>
                        <div>
                            <p class="text-zinc-500">Humans</p>
                            <p class="mt-1 text-lg font-semibold text-zinc-900">{{ number_format(data_get($admin_analytics, "periods.{$period}.human_visitors", 0)) }}</p>
                        </div>
                        <div>
                            <p class="text-zinc-500">Human vs bot</p>
                            <p class="mt-1 text-sm font-semibold text-zinc-900">{{ number_format((float) data_get($admin_analytics, "periods.{$period}.human_percentage", 0), 1) }}% / {{ number_format((float) data_get($admin_analytics, "periods.{$period}.bot_percentage", 0), 1) }}%</p>
                        </div>
                    </div>
                </div>
            @endforeach
        ...
        <div class="grid gap-6 xl:grid-cols-2">
            <div class="rounded-lg border border-zinc-200 p-4">
                ...
                <div class="mt-4 space-y-2 text-sm text-zinc-700">
                    @forelse(($admin_analytics['page_views_per_day'] ?? collect()) as $row)
            ...
            <div class="rounded-lg border border-zinc-200 p-4">
                ...
                <div class="mt-4 space-y-2 text-sm text-zinc-700">
                    @forelse(($admin_analytics['events_per_day'] ?? collect()) as $row)
                        <div class="flex items-center justify-between rounded-md bg-zinc-50 px-3 py-2">
                            <span>{{ \Illuminate\Support\Carbon::parse($row->day)->format('d M Y') }}</span>
                            <span class="font-medium">{{ number_format((int) $row->total) }}</span>
                        </div>
        ...
        <div class="grid gap-6 xl:grid-cols-2">
            <div class="rounded-lg border border-zinc-200 p-4">
                ...
                <div class="mt-4 space-y-2 text-sm text-zinc-700">
                    @forelse(($admin_analytics['top_page_types'] ?? collect()) as $row)
                        <div class="flex items-center justify-between rounded-md bg-zinc-50 px-3 py-2">
            ...
            <div class="rounded-lg border border-zinc-200 p-4">
                ...
                <div class="mt-4 space-y-2 text-sm text-zinc-700">
                    @forelse(($admin_analytics['top_landing_pages'] ?? collect()) as $row)
        ...
        <div class="grid gap-6 xl:grid-cols-2">
            <div class="rounded-lg border border-zinc-200 p-4">
                ...
                <div class="mt-4 space-y-2 text-sm text-zinc-700">
                    @forelse(($admin_analytics['top_events'] ?? collect()) as $row)
                        <div class="flex items-center justify-between rounded-md bg-zinc-50 px-3 py-2">
            ...
            <div class="rounded-lg border border-zinc-200 p-4">
                ...
                <div class="mt-4 space-y-2 text-sm text-zinc-700">
                    @forelse(($admin_analytics['top_ip_addresses'] ?? collect()) as $row)
                        <div class="flex items-center justify-between rounded-md bg-zinc-50 px-3 py-2">
        ...
        <div class="grid gap-6 xl:grid-cols-2">
            <div class="rounded-lg border border-zinc-200 p-4">
                ...
                <div class="mt-4 space-y-2 text-sm text-zinc-700">
                    @forelse(($admin_analytics['repeat_visitors_by_ip'] ?? collect()) as $row)
            ...
            <div class="rounded-lg border border-zinc-200 p-4">
                <div class="flex items-center justify-between">
                    ...
                    <span class="rounded-md bg-amber-50 px-2 py-1 text-xs font-medium text-amber-700">{{ number_format((float) data_get($admin_analytics, 'traffic_split.bot_percentage', 0), 1) }}% bots</span>
                </div>
                <div class="mt-4 grid gap-4 md:grid-cols-2">
                    <div class="rounded-md bg-zinc-50 px-3 py-3">
                        <p class="text-xs uppercase tracking-[0.18em] text-zinc-500">Human Visits</p>
                        <p class="mt-2 text-2xl font-semibold text-zinc-900">{{ number_format((int) ($admin_analytics['human_traffic_count'] ?? 0)) }}</p>
                    </div>
                    <div class="rounded-md bg-zinc-50 px-3 py-3">
                        <p class="text-xs uppercase tracking-[0.18em] text-zinc-500">Bot Visits</p>
                        <p class="mt-2 text-2xl font-semibold text-zinc-900">{{ number_format((int) ($admin_analytics['bot_traffic_count'] ?? 0)) }}</p>
                    </div>
                </div>
                <p class="mt-3 text-xs text-zinc-500">
                    {{ number_format((int) data_get($admin_analytics, 'traffic_split.human', 0)) }} humans / {{ number_format((int) data_get($admin_analytics, 'traffic_split.bots', 0)) }} bots in the selected window.
                </p>
            </div>
        </div>
        ...
        <div class="grid gap-6 xl:grid-cols-2">
            <div class="rounded-lg border border-zinc-200 p-4">
                <div class="flex items-center justify-between">
                    ...
                    <span class="rounded-md bg-amber-50 px-2 py-1 text-xs font-medium text-amber-700">{{ number_format((int) ($admin_analytics['bot_traffic_count'] ?? 0)) }} bot visits</span>
                </div>
                <div class="mt-4 space-y-2 text-sm text-zinc-700">
                    @forelse(($admin_analytics['suspicious_high_frequency_ips'] ?? collect()) as $row)
                        <div class="rounded-md bg-zinc-50 px-3 py-2">
                            <div class="flex items-center justify-between">
                                <span>{{ $row->ip_address }}</span>
            ...
            <div class="rounded-lg border border-zinc-200 p-4">
                ...
                <div class="mt-4 space-y-2 text-sm text-zinc-700">
                    @forelse(($admin_analytics['top_bot_names'] ?? collect()) as $row)
                        <div class="flex items-center justify-between rounded-md bg-zinc-50 px-3 py-2">
        ...
        <div class="grid gap-6 xl:grid-cols-2">
            <div class="rounded-lg border border-zinc-200 p-4">
                ...
                <div class="mt-4 space-y-2 text-sm text-zinc-700">
                    @forelse(($admin_analytics['top_bot_ips'] ?? collect()) as $row)
                        <div class="flex items-center justify-between rounded-md bg-zinc-50 px-3 py-2">
                            <span>{{ $row->ip_address }}</span>
                            <span class="font-medium">{{ number_format((int) $row->total_visits) }}</span>
                        </div>
            ...
            <div class="mt-4 overflow-x-auto">
                <table class="min-w-full divide-y divide-zinc-200 text-sm">
                    ...
                    <tbody class="divide-y divide-zinc-100">
                        @forelse(($admin_analytics['recent_visits'] ?? collect()) as $visit)
