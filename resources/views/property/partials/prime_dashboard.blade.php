...
(function(){
    ...
    function renderCharts() {
        ...
        const snapshotSalesYoyEl = document.getElementById('snapshot-sales-yoy');
        ...
        function snapshotForDistrict(district) {
            const districtCharts = charts[district] || {};
            const salesSeries = [...(districtCharts.sales || [])]
                .map((row) => ({ year: Number(row.year), sales: Number(row.sales) }))
                .sort((left, right) => left.year - right.year);
            const priceSeries = [...(districtCharts.avgPrice || [])]
                .map((row) => ({ year: Number(row.year), avg_price: Number(row.avg_price) }))
                .sort((left, right) => left.year - right.year);

            const currentSales = salesSeries.at(-1);
            const previousSales = salesSeries.length > 1 ? salesSeries.at(-2) : null;
            const currentPrice = priceSeries.at(-1);
            const previousPrice = priceSeries.length > 1 ? priceSeries.at(-2) : null;

            return {
                rolling_12_sales: Number(currentSales?.sales ?? 0),
                rolling_12_median_price: Number(currentPrice?.avg_price ?? 0),
                rolling_12_sales_yoy: percentageChange(currentSales?.sales, previousSales?.sales),
                rolling_12_price_yoy: percentageChange(currentPrice?.avg_price, previousPrice?.avg_price),
            };
        }

        function applySnapshotTone(element, value) {
            if (!element) {
                return;
            }

            element.classList.toggle('text-green-600', Number(value) >= 0);
            element.classList.toggle('text-red-600', Number(value) < 0);
        }

        function updateSnapshot(district) {
            const snapshot = district && charts[district]
                ? snapshotForDistrict(district)
                : initialSnapshot;

            if (snapshotSalesEl) {
                snapshotSalesEl.textContent = fmtNum(snapshot.rolling_12_sales ?? 0);
            }

            if (snapshotMedianPriceEl) {
                snapshotMedianPriceEl.textContent = `£${fmtNum(snapshot.rolling_12_median_price ?? 0)}`;
            }

            if (snapshotPriceYoyEl) {
                snapshotPriceYoyEl.textContent = fmtPct(snapshot.rolling_12_price_yoy ?? 0);
                applySnapshotTone(snapshotPriceYoyEl, snapshot.rolling_12_price_yoy ?? 0);
            }

            if (snapshotSalesYoyEl) {
                snapshotSalesYoyEl.textContent = fmtPct(snapshot.rolling_12_sales_yoy ?? 0);
                applySnapshotTone(snapshotSalesYoyEl, snapshot.rolling_12_sales_yoy ?? 0);
            }
        }
        ...
        function renderDistrict(district) {
            ...
            const yearRangeTitle = @json($rollingRangeTitle);
            const yoyRangeTitle = @json($rollingRangeTitle);
